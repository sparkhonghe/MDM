...
/*------------------------------------------------------*/
int readSequence (FILE *fil, int *conv, char **seq, char **id, char **desc, int *length) ;
						  ...
						  */
							 ...
							 ngc = ng + nc;
							  if (((imx+2)-(imn+2))>199 && (ngc*100.0/(imx+1-imn))>50.0 ) 
							  {
									...
									if ( obsToExp > 0.60 )
								   {
											printf("%s\t %d\t %d\t %d\t CpG: %d\t %.1f\t %.2f\t %.2f\n", seqname, imn+2, imx+2, mx, ncpg, ngc*100.0/(imx+1-imn), 1.0*ncpg/ngpc, obsToExp) ; 
											fprintf(fp,"%s\t %d\t %d\t %d\t CpG: %d\t %.1f\t %.2f\t %.2f\n", seqname, imn+2, imx+2, mx, ncpg, ngc*100.0/(imx+1-imn), 1.0*ncpg/ngpc, obsToExp);
								   }
							  }
